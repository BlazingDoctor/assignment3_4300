...
def solve_mrv(csp: CSP) -> Iterable[Assignment]:
    domains = {v: list(ds) for v, ds in csp.domains.items()}
    cons_by_var: Dict[str, List[Constraint]] = {v: [] for v in domains}
    for c in csp.constraints:
        for v in c.scope:
    ...
    def backtrack():
        ...
        # 1. Find all variables that are not yet assigned.
        unassigned_vars = [v for v in domains if v not in assignment]
        
        # 2. Select the one with the smallest current domain size.
        v = min(unassigned_vars, key=lambda var: len(domains[var]))
        # --- End of Heuristic ---

        # Try each value in the chosen variable's domain
        for val in domains[v]:
            assignment[v] = val
            if consistent_with_local(v, assignment):
                # Forward checking: prune domains of future (unassigned) variables
                pruned = []
                ok = True
                
                # Get the remaining unassigned variables for the forward check
                future_vars = [var for var in unassigned_vars if var != v]

                for w in future_vars:
                    removed = []
                    for vv in list(domains[w]):
                        assignment[w] = vv # Temporarily assign to check
                        if not consistent_with_local(w, assignment):
                            domains[w].remove(vv)
                            removed.append(vv)
                        del assignment[w] # Un-assign
                    if removed:
                        pruned.append((w, removed))
                    # If a domain becomes empty, this path is not viable
                    if not domains[w]:
                        ok = False
                        break
                ...
                # Backtrack: undo the pruning
                for w, removed in pruned:
                    domains[w].extend(removed)
            
            del assignment[v] # Un-assign and backtrack
